...
class telescope:
    ...
    def send(self,cmd, timeout=5):
        ...
        try:
            ...
            self.logger.error("Error connecting to " + self.server + ':' + str(self.port))
            return None
    ...
    def offset_target_object(self,east,north):
        ...
        if math.abs(east) > self.max_jog or math.abs(north) > self.max_jog:
            self.logger.error('Cannot jog more than ' + str(self.max_jog) + '" in any direction')
        
        self.send("#7," + str(east) + "," + str(north) + ";")
    ...
    def read_tcs_status(self):
        ...
        if len(arr) == 8:
            ...
            status['drive_switch_up']      = byte0[0]=='1'
            status['track_switch_up']      = byte0[1]=='1'
            status['aux_track_switch_up']  = byte0[2]=='1'
            status['dome_track_switch_up'] = byte0[3]=='1'
            status['auto_dome_switch_up']  = byte0[4]=='1'
            status['amplifiers_switch_up'] = byte0[5]=='1'

            byte1 = format(int(arr[1]),'b').zfill(8)
            status['north_button_pressed'] = byte1[0]=='1'
            status['south_button_pressed'] = byte1[1]=='1'
            status['east_button_pressed']  = byte1[2]=='1'
            status['west_button_pressed']  = byte1[3]=='1'
            status['set_button_pressed']   = byte1[4]=='1'
            status['slew_button_pressed']  = byte1[5]=='1'
            status['ascom_guiding']        = byte1[6]=='1'

            byte2 = format(int(arr[2]),'b').zfill(8)
            status['horizon_limit_reached']  = byte2[0]=='1'
            status['east_travel_limit']      = byte2[1]=='1'
            status['west_travel_limit']      = byte2[2]=='1'
            status['north_travel_limit']     = byte2[3]=='1'
            status['south_travel_limit']     = byte2[4]=='1'
            status['target_out_of_range']    = byte2[5]=='1'
            status['approaching_soft_limit'] = byte2[6]=='1'
            status['reached_soft_limit']     = byte2[7]=='1'

            byte3 = format(int(arr[3]),'b').zfill(8)
            status['find_home_mode'] = byte3[0]=='1'
            status['goto_az_mode']   = byte3[1]=='1'
            status['track_mode']     = byte3[2]=='1'
            status['rotating_left']  = byte3[3]=='1'
            status['rotating_right'] = byte3[4]=='1'
            status['at_home']        = byte3[5]=='1'
            status['manual_mode']    = byte3[6]=='1'

            byte4 = format(int(arr[4]),'b').zfill(8)
            status['focus_moving_in']        = byte4[0]=='1'
            status['focus_moving_out']       = byte4[1]=='1'
            status['focus_at_outer_limit']   = byte4[2]=='1'
            status['focus_at_inner_limit']   = byte4[3]=='1'
            status['instrument_rotator_ccw'] = byte4[4]=='1'
            status['instrument_rotator_cw']  = byte4[5]=='1'

            byte5 = format(int(arr[5]),'b').zfill(8)
            status['serial_comm_ready']        = byte5[0]=='1'
            status['tcpip_connected']          = byte5[1]=='1'
            status['use_cos_dec']              = byte5[2]=='1'
            status['use_rate_corrections']     = byte5[3]=='1'
            status['east_side_pier_operation'] = byte5[4]=='1'
            status['west_side_pier_operation'] = byte5[5]=='1'
            status['galil_data_is_stale']      = byte5[6]=='1'

            byte6 = format(int(arr[6]),'b').zfill(8)
            status['mount_in_chase_target_mode']   = byte6[0]=='1'
            status['mount_in_slew_to_target_mode'] = byte6[1]=='1'
            status['mount_in_follow_target_mode']  = byte6[2]=='1'
            status['mount_in_hold_position_mode']  = byte6[3]=='1'
            status['target_captured']              = byte6[4]=='1'
            
            byte7 = format(int(arr[7]),'b').zfill(8)
            status['dome_upper_shutter_open']   = byte7[0]=='1'
            status['dome_upper_shutter_closed'] = byte7[1]=='1'
            status['dome_lower_shutter_open']   = byte7[2]=='1'
            status['dome_lower_shutter_closed'] = byte7[3]=='1'
            status['mirror_doors_open']         = byte7[4]=='1'
            status['mirror_doors_closed']       = byte7[5]=='1'
...
if __name__ == '__main__':
    ...
    if socket.gethostname() == 'tres-guider':
        ...
        base_directory = '/home/observer/tres/'
    else:
    ...
    status = telscope.read_tcs_status()
